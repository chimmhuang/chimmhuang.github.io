...
本项目是一个**人体成分分析报告**展示系统，基于小米运动健康数据进行可视化展示。系统通过读取 `health_data.json` 文件，将健康数据渲染到 HTML 页面中，展示用户的身体指标、身体成分、评估指标、趋势图表和运动日历等信息。
...
| `score` | Object | 健康评分与建议 |
| `bodyMetrics` | Object | 身体指标（体重、BMI、体脂率、心率） |
| `bodyComposition` | Object | 身体成分（水分、蛋白质、脂肪、骨盐量等） |
| `assessment` | Object | 评估指标（身体形态、腰臀比等） |
| `segmentalFat` | Object | 节段脂肪分析 |
| `segmentalMuscle` | Object | 节段肌肉分析 |
| `history` | Array | 历史记录（用于趋势图表） |
| `activities` | Array | 活动记录（用于日历展示） |
...
| `measurementDate` | String | 测量日期（ISO格式） | "2026-05-26" |
...
| `bodyAge` | Number | 身体年龄 | 26 | `assess-age` |
| `skeletalMuscleIndex` | Number | 骨骼肌指数 | 8.3 | `assess-skeletal` |
| `suggestedCalorieIntake` | Number | 建议热量摄入（kcal） | 2504 | `assess-calorie` |

**页面映射**：`index.html` 评估指标卡片

---

### 2.7 segmentalFat / segmentalMuscle - 节段分析
...
| `bad_eating` | 不良饮食 | `.bad_eating`（橙色/警告背景） |
...
1. **profile.measurementDate** - 更新为最新测量日期
2. **bodyMetrics** - 更新各项指标的 `value` 和 `level`
3. **bodyComposition** - 更新各项成分的 `value` 和 `level`
4. **assessment** - 更新评估指标
5. **segmentalFat / segmentalMuscle** - 更新节段数据
6. **history** - 添加新的历史记录（追加到数组末尾）
7. **activities** - 添加新的活动记录

### 4.2 注意事项

- 日期格式必须为 `YYYY-MM-DD`（ISO 8601 格式）
- `history` 和 `activities` 数组按日期升序排列
- `level` 字段值必须与 `range` 数组中的值一致，否则样式可能显示异常
- 体重单位为 **kg**，前端展示时会自动转换为 **斤**（×2）

---

## 5. 数据源说明

本项目数据来源于**小米运动健康 App**，通过导出功能获取人体成分分析数据，手动整理为 JSON 格式。
